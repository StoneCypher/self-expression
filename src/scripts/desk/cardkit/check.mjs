...
export function faults(name, mod) {
  ...
  /* Optional, but strict once declared. `forms` is how a type says it draws more than one picture —
     `chart` is five plots, `render_diagram` is four — because the gallery can only show what a type
     declares, and without this half the catalogue's real vocabulary is invisible on the one surface
     built to teach it. Two is the floor: a type with a single form has no forms, it has itself, and
     a one-entry list would put a redundant twig under every leaf in the tree. */
  if (meta.forms !== undefined) {
    if (!Array.isArray(meta.forms)) {
      bad.push(`meta.forms must be an array, got ${typeof meta.forms}`);
    } else if (meta.forms.length < 2) {
      bad.push(`meta.forms has ${meta.forms.length} entr${meta.forms.length === 1 ? 'y' : 'ies'}; ` +
               'a type with fewer than two forms should omit it');
    } else {
      const seen = new Set();
      for (const [i, f] of meta.forms.entries()) {
        const at = `meta.forms[${i}]`;
        if (!f || typeof f !== 'object') { bad.push(`${at} is not an object`); continue; }
        if (typeof f.name !== 'string' || !f.name.trim()) bad.push(`${at}.name missing`);
        else if (seen.has(f.name)) bad.push(`${at}.name "${f.name}" is declared twice`);
        else seen.add(f.name);
        if (typeof f.summary !== 'string' || !f.summary.trim()) bad.push(`${at}.summary missing`);
        else if (!f.summary.trim().endsWith('.')) bad.push(`${at}.summary does not end in a period`);
        /* `via` is a STRING for the same reason `shape` is: it is read by a person deciding how to
           ask for this form, so it must be the literal they would write. */
        if (f.via !== undefined && (typeof f.via !== 'string' || !f.via.trim()))
          bad.push(`${at}.via must be a non-empty string when present`);
      }
    }
  }
  ...
  /* Only in what is RENDERED. Emitted script legitimately contains the words: a card that guards
     ...
     page. */
  /* Leading boundary only. Anchoring BOTH ends missed the shape a leaked value actually takes:
     it is almost never alone, it is concatenated onto something — `NaNsprint`, `Infinitydays`,
     `NaN%` — and a trailing `\b` requires a non-word character after it, which a glued suffix does
     not provide. Found by an agent whose own suite escaped exactly this mutation. */
  if (/\bNaN|\bInfinity/.test(html)) bad.push('html contains NaN or Infinity');
 ...
 *
 * Runs on EVERY invocation, not only under `--selftest`, because the sentence above was a claim the
 * wiring did not honour. Quiet on success so a caller parsing the sweep's output sees no change.
 *
 * @param quiet suppress the passing summary line; failures always speak
 * @returns the number of mutations that escaped, which should be zero
 */
function selftest(quiet) {
  const sound = () => ({
    meta: { name: 'probe', summary: 'A conforming probe type.', shape: '{ x }',
            category: 'work-and-lists', defaults: { a: 1 } },
    build: () => ({
      json: { ord: 50 },
  ...
  const breaks = [
    ...
    /* The category rule arrived without a mutation, so it was the one rule in `faults` that
       nothing proved. Two of them: absent, and present but not a real key — a typo in a category
       is likelier than an omission and lands in the uncategorised bin just the same. */
    ['category missing',   m => { delete m.meta.category; }],
    ['category bogus',     m => { m.meta.category = 'charts'; }],
    ...
    /* Glued to a suffix, which is the shape a leaked value actually takes — a bare `NaN` between
       spaces is the easy case and was the only one tested. This is the mutation that escaped. */
    ['NaN glued in html',  m => { const b = m.build; m.build = a => { const r = b(a); r.html = r.html.replace('<h2>p', '<h2>NaNsprint'); return r; }; }],
  ];
...
if (only === '--selftest') { process.exit(selftest(false) ? 1 : 0); }

/* Actually before anything else, now.
   The DocBlock above has always said "this check proves itself before it judges anything else",
   and it did not: the selftest ran only under `--selftest`, so every ordinary sweep judged the
   whole catalogue without ever establishing that the judge works. A comment asserting a property
   the code does not have is worse than no comment, because it is believed.
   Silent on success, so the output a caller parses is unchanged; loud and fatal on failure,
   because a checker that cannot catch a planted fault has no business reporting zero. */
if (selftest(true)) {
  console.log('✗ the checker failed its own selftest — refusing to judge the catalogue.');
  console.log('  run `node check.mjs --selftest` for the detail.');
  process.exit(1);
}
