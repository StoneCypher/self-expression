...
export const meta = {
  ...
  /* Five plots under one name, which is why a reader looking for a scatter plot could not find one:
     the catalogue lists types, and four fifths of this type's vocabulary had no entry anywhere.
     Declaring them is the fix that does not cost five copies of the axis, tick and label code.

     `stackedbar` is deliberately NOT here — it is its own type, because a stacked bar answers
     `part-of-a-whole` while this answers `ranking-and-comparison`, and a category belongs to a
     type. That is the line: a different QUESTION is a new type, a different PICTURE is a form. */
  forms: [
    { name: 'line',    via: "kind: 'line'",
      summary: 'A line per series over a shared x, for a quantity moving continuously.' },
    { name: 'area',    via: "kind: 'area'",
      summary: 'A line with the region beneath it filled, which reads as accumulated quantity.' },
    { name: 'bar',     via: "kind: 'bar'",
      summary: 'Horizontal bars from a zero baseline, which suit long category names.' },
    { name: 'column',  via: "kind: 'column'",
      summary: 'Vertical bars from a zero baseline, the usual form when x is ordered.' },
    { name: 'scatter', via: "kind: 'scatter'",
      summary: 'One mark per point with no connection implied, for unordered observations.' },
  ],
};
