...
export const meta = {
  ...
  /* Three layouts, declared rather than split into three types. They answer ONE question — what
     connects to what — so they share a category, and a type is what a category indexes. But each
     has a name a reader would search for, and a form that nobody can find is a form nobody uses.
     Splitting instead would triple the node normalisation, label placement, edge routing, self-loop
     and parallel-edge handling — and being able to flip layouts on the SAME data is itself the
     finding: a graph that reads as structure under a circular order and as mush under force has
     told you something no single layout could. */
  forms: [
    { name: 'force',   via: "layout: 'force'",
      summary: 'A seeded force simulation, run at build time so it draws the same picture every load.' },
    { name: 'layered', via: "layout: 'layered'",
      summary: 'Longest-path layering, for a graph with a direction of flow through it.' },
    { name: 'circle',  via: "layout: 'circle'",
      summary: 'Nodes on a ring in a crossing-reducing order, which makes density legible.' },
  ],
};
