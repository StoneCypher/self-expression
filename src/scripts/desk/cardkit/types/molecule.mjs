...
export const meta = {
  ...
  /* Flow-and-relationship, not live-and-ambient. Its data is `bonds: [[i, j, order]]` — an edge
     list, the same primitive `graph` and `arc` eat — and a structural formula answers literally
     "what connects to what". Nothing about it is true only right now; it is true always, which is
     the opposite of what live-and-ambient means. It rotates, but motion is a rendering choice
     rather than a question, and the taxonomy sorts by question. */
...
/**
 * A caller-supplied name made safe to sit inside a block comment in the emitted script.
 *
 * The emitted script names its molecule in its opening comment, and comments SHIP: they are
 * concatenated into the one inline block every card's script shares. So the name is data
 * arriving in a context where escaping is not available — a comment has no escape sequences,
 * only sequences that end it or that a scanner refuses. Each becomes the same characters with a
 * space wedged in, which still reads as the original and can no longer be mistaken for the
 * token: the comment terminator; a backtick, which would end the template literal this script
 * is assembled inside; an optional chain; an arrow; and `</`, because an HTML parser ends a
 * `<script>` element at `</script` wherever it appears, comment or not — the one hazard here
 * that escaping genuinely cannot reach, since a comment has nowhere to put an escape.
 *
 * Control characters, DEL and the two line separators become spaces, compared numerically
 * rather than matched against a character class, because writing the class is how the class
 * gets corrupted.
 *
 * @param s the name as the caller wrote it
 * @returns the same text with every comment-hostile sequence broken by a space
 *
 * @example commentSafe('why?.because');     // 'why? .because'
 * @example commentSafe('a</script>b');      // 'a< /script>b'
 * @example commentSafe('water');            // 'water'
 */
function commentSafe(s) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    out += (c < 32 || c === 127 || c === 0x2028 || c === 0x2029) ? ' ' : text[i];
  }
  return out
    .replace(/\*\//g, '* /')
...
function script(id, mol) {
  /* The same escape set every other type's literal helper uses, and for the same reasons. `<` so
     a name holding `</script` cannot close the element early; `>` with it, so no name can put an
     arrow into a file that is contractually free of them; `?` so a name reading "ready?.no"
     cannot look like optional chaining to a guard that scans the emitted text raw; the backtick,
     reached for by code point rather than typed, because this script is assembled inside a
     template literal and one would end it; and the two line separators, which are newlines to a
     JS parser and not to `JSON.stringify`. Every one decodes back to itself, so nothing the
     viewer reads changes. */
  const payload = JSON.stringify(mol)
    .replace(/</g, '\\u003c').replace(/>/g, '\\u003e')
    .replace(/\?/g, '\\u003f')
    .replace(new RegExp(String.fromCharCode(96), 'g'), '\\u0060')
    .replace(/\u2028/g, '\\u2028').replace(/\u2029/g, '\\u2029');
  ...
  return `
/* ${commentSafe(mol.name)} as ball-and-stick, hand-rolled: the desk is served under script-src 'self',
