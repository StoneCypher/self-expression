...
export function buildPlan(opts = {}) {
  ...
  // Only report a feature as "disabled" if it was on by default but ended up
  // off.  Features that are off-by-default (defaultEnabled: false) remaining
  // off are simply "not enabled" and don't belong in the disabled list.
  const disabled = OPTIONAL_FEATURE_NAMES.filter(
    n => merged[n] === false && FEATURES[n].defaultEnabled === true
  );
