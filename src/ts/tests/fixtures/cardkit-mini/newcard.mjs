...
/* The deck is the desk's `cards/` directory. A desk is deliberately not a default location
   (see panel.mjs), so this follows the same rule: the env var when set, else the cwd. */
const DECK  = join(process.env.SELF_EXPRESSION_DESK ?? process.cwd(), 'cards');
...
export function writeCard(mod, spec, deck) {
  ...
  /* The category travels with the instance, not only with the type. A card on disk should be able
     to say what kind of question it answers without anyone loading its module — which is what lets
     a deck be sorted, grouped or audited by category from `card.json` alone.

     `spec` travels for a harder reason. A card is a SNAPSHOT: `build` runs once and its output is
     frozen into three files, so improving a type does nothing at all to the cards already on a
     desk. Without the seed recorded there is no way back — the data was absorbed into the markup
     and cannot be recovered from it — so every installed card was permanently un-regenerable, and
     nobody noticed until a type was rewritten and the desk did not change. It is written after the
     spread so a type cannot shadow its own provenance. */
  writeFileSync(join(dir, 'card.json'),
                JSON.stringify({ ord: spec.ord, type: mod.meta?.name,
                                 category: mod.meta?.category ?? null, ...meta,
                                 spec: { title: spec.title ?? spec.id,
                                         data:  spec.data ?? null } }, null, 2) + '\n');
  return dir;
}

/**
 * Re-run every installed card through the current version of its type.
 *
 * A card is frozen at install time, so a type improved afterwards reaches nothing. This is how an
 * improvement reaches a desk that already exists.
 *
 * A card written before `writeCard` recorded its `spec` is **refused rather than rebuilt**. The
 * temptation is to rebuild it with no data, which is silently destructive: a seeded card would
 * come back as an empty one and look like a rendering bug rather than a data loss. Naming it and
 * leaving it alone is the only honest option — it needs one manual reinstall, after which it
 * carries its own seed forever.
 *
 * @param cat  a loaded catalogue, as {@link catalogue} yields
 * @param deck the deck directory to walk
 * @returns `{ rebuilt, skipped, failed }`, each a list of `{ id, why? }`
 *
 * @example
 * const r = rebuildDeck(await catalogue(), DECK);
 * r.rebuilt.length;   // 12
 */
export function rebuildDeck(cat, deck) {
  const out = { rebuilt: [], skipped: [], failed: [] };
  let entries = [];
  try { entries = readdirSync(deck, { withFileTypes: true }).filter(e => e.isDirectory()); }
  catch { return out; }

  for (const e of entries) {
    const id = e.name;
    let card;
    try { card = JSON.parse(readFileSync(join(deck, id, 'card.json'), 'utf8')); }
    catch { out.skipped.push({ id, why: 'no readable card.json' }); continue; }

    const mod = cat.get(card.type);
    if (!mod) { out.skipped.push({ id, why: `unknown type ${JSON.stringify(card.type)}` }); continue; }
    if (!card.spec) {
      out.skipped.push({ id, why: 'installed before seeds were recorded — reinstall it once by hand' });
      continue;
    }

    const spec = { id, title: card.spec.title, data: card.spec.data ?? undefined, ord: card.ord };
    try {
      const complaints = audit(mod.build({ ...spec }), { contains: mod.meta?.contains === true });
      if (complaints.length) { out.failed.push({ id, why: complaints.join('; ') }); continue; }
      writeCard(mod, spec, deck);
      out.rebuilt.push({ id });
    } catch (err) { out.failed.push({ id, why: err.message }); }
  }
  return out;
}
 ...
 *
 * This must not be weaker than `check.mjs`'s `faults`, and it was, in two ways an agent found by
 * hitting them: the keyword scan was anchored to the start of a line, so an INDENTED `const`
 * installed cleanly and then failed the catalogue check; and the control-character scan stopped at
 * 0x1f, missing DEL, which `JSON.stringify` does not escape either — so a label holding one landed
 * raw in the emitted script. An installer that accepts what the checker rejects is worse than no
 * installer check, because it moves the failure to whoever runs the sweep next.
 *
 * @param built the `{ html, css, js }` a type produced
 * @param opts  `{ contains }` — true for a type that legitimately emits several sections
 * @returns a list of complaints, empty when the card is safe to install
 *
 * @example
 * audit({ js: 'var f = () => 1;' });   // ['js: contains an arrow function']
 * @example
 * audit({ js: 'var a = 1;', html: '<section data-card="x"></section>' });   // []
 */
export function audit(built, opts = {}) {
  ...
  /* Line-anchored, which catches an indented declaration but NOT one tucked inside a brace on a
     line of its own code (`if (x) { const y = 1; }`). Scanning anywhere would catch that and would
     also fire on the word `const` inside a comment or a string, and a guard that cries wolf is a
     guard that gets switched off — one card was already refused because a comment said "the class
     is what CSS reads". `check.mjs` gets this right by blanking comments and string bodies first,
     which is the routine both should share; it lives there, and `check.mjs` already imports THIS
     file, so lifting it out is a three-file change and not one to make while agents are running
     the checker. Until then this is deliberately the weaker of the two, and the catalogue sweep is
     what closes the gap. */
  if (/^\s*(const|let)\s/m.test(js)) bad.push('js: declares const/let at the start of a line');

  /* Every byte below 0x20 that is not tab, LF or CR, plus DEL. Compared numerically rather than
     matched against a character class, because writing the class is how the class gets corrupted —
     seven separate incidents in one evening, one of them inside the warning about it.

     DEL was missing here and present in `check.mjs`, so a card holding one installed cleanly and
     then failed the sweep. It is worth naming separately because it is the one control character
     `JSON.stringify` does NOT escape: a label carrying it survives serialisation intact and lands
...
} else if (argv[0] === 'rebuild') {
  const deck = flag('deck', DECK);
  const r = rebuildDeck(await catalogue(), deck);
  for (const { id } of r.rebuilt) console.log(`  rebuilt  ${id}`);
  for (const { id, why } of r.skipped) console.log(`  skipped  ${id} — ${why}`);
  for (const { id, why } of r.failed)  console.error(`  FAILED   ${id} — ${why}`);
  console.log(`${r.rebuilt.length} rebuilt, ${r.skipped.length} skipped, ${r.failed.length} failed`);
