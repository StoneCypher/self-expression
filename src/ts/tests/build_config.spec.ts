  ...
  it('with all-on base config, produces every feature in correct stages', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: {
          a11y_coverage: true, docs: true, eslint: true, cloc: true, changelog: true,
    ...
    try {
      ...
      expect(stages[1]?.sort()).toEqual(
        ['a11y_coverage', 'changelog', 'cloc', 'docs', 'eslint', 'just_test_save', 'typescript'].sort()
      );
...
describe('buildPlan — profiles', () => {
  const allOnConfig = {
    ...
    profiles: {
      fast: {
        features: {
          a11y_coverage: false, docs: false, eslint: false, cloc: false, changelog: false,
  ...
  it('applies a profile selected via env var', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['a11y_coverage', 'docs', 'eslint', 'cloc', 'changelog', 'viz_png', 'attw', 'site'].sort()
      );
    ...
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        ...
        profiles: {
          ...
          ci: { features: { a11y_coverage: true, docs: true, eslint: true, cloc: true, changelog: true,
                            viz_png: true, terser: true, attw: true, site: true } },
        },
      }),
    });
...
describe('buildPlan — file cascade', () => {
  it('overlays build.config.<env>.json on top of base', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { a11y_coverage: true, docs: true, eslint: true, cloc: true, changelog: true,
                    viz_png: true, terser: true, attw: true, site: true },
      }),
...
describe('buildPlan — env-var and CLI feature overrides', () => {
  ...
  it('--only restricts to listed features, disabling all other optionals', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['a11y_coverage', 'attw', 'changelog', 'cloc', 'docs', 'site', 'terser', 'viz_png'].sort()
      );
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
...
describe('buildPlan — --enable positive test', () => {
  it('--enable lifts a feature that the base config set to false', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { a11y_coverage: false, docs: false, eslint: true, cloc: true, changelog: true,
                    viz_png: true, terser: true, attw: true, site: true },
      }),
    });
    try {
      const { disabled } = buildPlan({ cwd, argv: ['--enable=docs'], env: {} });
      expect(disabled).toEqual(['a11y_coverage']);
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
});

describe('buildPlan — a11y_coverage feature (stub, default off)', () => {
  const baseAllOn = {
    features: { docs: true, eslint: true, cloc: true, changelog: true,
                viz_png: true, terser: true, attw: true, site: true },
  };

  it('is disabled by default (stub)', () => {
    const cwd = makeTmpRepo({ 'build.config.json': JSON.stringify(baseAllOn) });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages.flat()).not.toContain('a11y_coverage');
      expect(disabled).toContain('a11y_coverage');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });

  it('runs in stage 1 when explicitly enabled', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { ...baseAllOn.features, a11y_coverage: true },
      }),
    });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages[1]).toContain('a11y_coverage');
      expect(disabled).not.toContain('a11y_coverage');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
});
