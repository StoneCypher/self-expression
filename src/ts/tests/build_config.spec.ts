    ...
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: {
          ...
          viz_png: true, terser: true, attw: true, site: true, e2e: true,
        },
      }),
    });
    try {
      ...
      expect(stages[6]).toEqual(['hosted_test']);
    ...
    profiles: {
      fast: {
        features: {
          ...
          viz_png: false, attw: false, site: false, e2e: false,
        },
      },
    },
  };
  ...
  it('applies a profile selected via env var', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['docs', 'eslint', 'cloc', 'changelog', 'viz_png', 'attw', 'site', 'e2e'].sort()
      );
...
describe('buildPlan — file cascade', () => {
  ...
  it('overlays build.config.local.json on top of env file', () => {
    ...
    try {
      const { disabled } = buildPlan({ cwd, argv: ['--env=ci'], env: {} });
      expect(disabled.sort()).toEqual(['docs', 'site', 'e2e'].sort());
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  });
...
describe('buildPlan — env-var and CLI feature overrides', () => {
  ...
  it('BUILD_DISABLE turns off listed features', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(['docs', 'eslint', 'site', 'e2e'].sort());
  ...
  it('--only restricts to listed features, disabling all other optionals', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['attw', 'changelog', 'cloc', 'docs', 'site', 'terser', 'viz_png', 'e2e'].sort()
      );
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
...
describe('buildPlan — dependency cascade', () => {
  ...
  it('auto-disables site when docs is disabled and emits a warning', () => {
    ...
    try {
      ...
      expect(warnings.join('\n')).toMatch(/auto-disabling.*e2e.*site/);
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
  ...
  it('does not warn when a dependent is already explicitly disabled', () => {
    ...
    try {
      const { warnings } = buildPlan({
        cwd, argv: ['--disable=docs,site,e2e'], env: {},
      });
...
describe('buildPlan — e2e feature', () => {
  const baseWithE2e = {
    features: { docs: true, eslint: true, cloc: true, changelog: true,
  ...
  it('places e2e (hosted_test script) in stage 6 when enabled', () => {
    const cwd = makeTmpRepo({ 'build.config.json': JSON.stringify(baseWithE2e) });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages[6]).toEqual(['hosted_test']);
      expect(disabled).not.toContain('e2e');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });

  it('auto-disables e2e when site is disabled', () => {
    const cwd = makeTmpRepo({ 'build.config.json': JSON.stringify(baseWithE2e) });
    try {
      const { disabled, warnings } = buildPlan({
        cwd, argv: ['--disable=site'], env: {},
      });
      expect(disabled.sort()).toEqual(['e2e', 'site'].sort());
      expect(warnings.join('\n')).toMatch(/auto-disabling e2e.*site/);
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });

  it('excludes e2e when explicitly disabled', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { ...baseWithE2e.features, e2e: false },
      }),
    });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages.flat()).not.toContain('hosted_test');
      expect(disabled).toContain('e2e');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
});
