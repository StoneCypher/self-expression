    ...
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: {
          ...
          type_coverage: true, viz_png: true, terser: true, attw: true, site: true,
        },
      }),
    });
    try {
      ...
      expect(stages[1]?.sort()).toEqual(
        ['changelog', 'cloc', 'docs', 'eslint', 'just_test_save', 'type_coverage', 'typescript'].sort()
      );
    ...
    profiles: {
      fast: {
        features: {
          ...
          type_coverage: false, viz_png: false, attw: false, site: false,
        },
      },
    },
  };
  ...
  it('applies a profile selected via env var', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['docs', 'eslint', 'cloc', 'changelog', 'type_coverage', 'viz_png', 'attw', 'site'].sort()
      );
...
describe('buildPlan — env-var and CLI feature overrides', () => {
  ...
  it('--only restricts to listed features, disabling all other optionals', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['attw', 'changelog', 'cloc', 'docs', 'site', 'terser', 'type_coverage', 'viz_png'].sort()
      );
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
...
describe('buildPlan — type_coverage feature', () => {
  const baseWithType = {
    features: { docs: true, eslint: true, cloc: true, changelog: true,
                type_coverage: true, viz_png: true, terser: true, attw: true, site: true },
  };

  it('includes type_coverage in stage 1 when enabled', () => {
    const cwd = makeTmpRepo({ 'build.config.json': JSON.stringify(baseWithType) });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages[1]).toContain('type_coverage');
      expect(disabled).not.toContain('type_coverage');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });

  it('excludes type_coverage when disabled', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { ...baseWithType.features, type_coverage: false },
      }),
    });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages.flat()).not.toContain('type_coverage');
      expect(disabled).toContain('type_coverage');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
});
