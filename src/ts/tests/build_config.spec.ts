...
describe('buildPlan — defaults', () => {
  it('with all-on base config, produces every feature in correct stages', () => {
    ...
    try {
      ...
      expect(disabled).toEqual(['perf_coverage']);
  ...
  it('applies a profile selected via env var', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['docs', 'eslint', 'cloc', 'changelog', 'viz_png', 'attw', 'site', 'perf_coverage'].sort()
      );
...
describe('buildPlan — file cascade', () => {
  it('overlays build.config.<env>.json on top of base', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(['eslint', 'perf_coverage'].sort());
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('overlays build.config.local.json on top of env file', () => {
    ...
    try {
      const { disabled } = buildPlan({ cwd, argv: ['--env=ci'], env: {} });
      expect(disabled.sort()).toEqual(['docs', 'perf_coverage', 'site'].sort());
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  });
...
describe('buildPlan — env-var and CLI feature overrides', () => {
  ...
  it('BUILD_DISABLE turns off listed features', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(['docs', 'eslint', 'perf_coverage', 'site'].sort());
  ...
  it('--only restricts to listed features, disabling all other optionals', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['attw', 'changelog', 'cloc', 'docs', 'perf_coverage', 'site', 'terser', 'viz_png'].sort()
      );
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
...
describe('buildPlan — perf_coverage feature (stub, default off)', () => {
  const baseAllOn = {
    features: { docs: true, eslint: true, cloc: true, changelog: true,
                viz_png: true, terser: true, attw: true, site: true },
  };
  ...
  it('is disabled by default (stub)', () => {
    ...
    try {
      ...
      expect(stages.flat()).not.toContain('perf_coverage');
      expect(disabled).toContain('perf_coverage');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
  ...
  it('runs in stage 1 when explicitly enabled', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { ...baseAllOn.features, perf_coverage: true },
      }),
    });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages[1]).toContain('perf_coverage');
      expect(disabled).not.toContain('perf_coverage');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
});
