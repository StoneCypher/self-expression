  ...
  it('with all-on base config, produces every feature in correct stages', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: {
          docs: true, doc_coverage: true, eslint: true, cloc: true, changelog: true,
    ...
    try {
      ...
      expect(stages[1]?.sort()).toEqual(
        ['changelog', 'cloc', 'doc_coverage', 'eslint', 'just_test_save', 'typescript'].sort()
      );
...
describe('buildPlan — profiles', () => {
  const allOnConfig = {
    ...
    profiles: {
      fast: {
        features: {
          docs: false, doc_coverage: false, eslint: false, cloc: false, changelog: false,
  ...
  it('applies a profile selected via env var', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['docs', 'doc_coverage', 'eslint', 'cloc', 'changelog', 'viz_png', 'attw', 'site'].sort()
      );
    ...
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        ...
        profiles: {
          ...
          ci: { features: { docs: true, doc_coverage: true, eslint: true, cloc: true,
                            changelog: true, viz_png: true, terser: true, attw: true, site: true } },
        },
      }),
    });
    try {
...
describe('buildPlan — file cascade', () => {
  it('overlays build.config.<env>.json on top of base', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { docs: true, doc_coverage: true, eslint: true, cloc: true,
                    changelog: true, viz_png: true, terser: true, attw: true, site: true },
      }),
...
describe('buildPlan — env-var and CLI feature overrides', () => {
  ...
  it('--only restricts to listed features, disabling all other optionals', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['attw', 'changelog', 'cloc', 'doc_coverage', 'docs', 'site', 'terser', 'viz_png'].sort()
      );
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
...
describe('buildPlan — --enable positive test', () => {
  it('--enable lifts a feature that the base config set to false', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { docs: false, doc_coverage: true, eslint: true, cloc: true,
                    changelog: true, viz_png: true, terser: true, attw: true, site: true },
      }),
    });
    try {
...
describe('buildPlan — doc_coverage feature', () => {
  const baseAllOnWithDocCov = {
    features: { docs: true, doc_coverage: true, eslint: true, cloc: true,
                changelog: true, viz_png: true, terser: true, attw: true, site: true },
  };

  it('includes doc_coverage in stage 1 (not docs)', () => {
    const cwd = makeTmpRepo({ 'build.config.json': JSON.stringify(baseAllOnWithDocCov) });
    try {
      const { stages } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages[1]).toContain('doc_coverage');
      expect(stages[1]).not.toContain('docs');
      expect(stages[4]).toContain('docs');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });

  it('excludes doc_coverage when disabled', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { ...baseAllOnWithDocCov.features, doc_coverage: false },
      }),
    });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages.flat()).not.toContain('doc_coverage');
      expect(disabled).toContain('doc_coverage');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
});
