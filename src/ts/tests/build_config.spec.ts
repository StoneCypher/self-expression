  ...
  it('with all-on base config, produces every feature in correct stages', () => {
    ...
    try {
      ...
      expect(stages[1]?.sort()).toEqual(
        ['changelog', 'cloc', 'docs', 'eslint', 'just_test_save', 'license_check', 'typescript'].sort()
      );
...
describe('buildPlan — profiles', () => {
  const allOnConfig = {
    features: {
      docs: true, eslint: true, cloc: true, changelog: true,
      license_check: true, viz_png: true, terser: true, attw: true, site: true,
    },
    profiles: {
      fast: {
        features: {
          docs: false, eslint: false, cloc: false, changelog: false,
          license_check: false, viz_png: false, attw: false, site: false,
        },
      },
    },
  };
  ...
  it('applies a profile selected via env var', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['docs', 'eslint', 'cloc', 'changelog', 'license_check', 'viz_png', 'attw', 'site'].sort()
      );
...
describe('buildPlan — env-var and CLI feature overrides', () => {
  ...
  it('--only restricts to listed features, disabling all other optionals', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['attw', 'changelog', 'cloc', 'docs', 'license_check', 'site', 'terser', 'viz_png'].sort()
      );
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
...
describe('buildPlan — license_check feature', () => {
  const baseWithLicense = {
    features: { docs: true, eslint: true, cloc: true, changelog: true,
                license_check: true, viz_png: true, terser: true, attw: true, site: true },
  };

  it('includes license_check in stage 1 when enabled', () => {
    const cwd = makeTmpRepo({ 'build.config.json': JSON.stringify(baseWithLicense) });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages[1]).toContain('license_check');
      expect(disabled).not.toContain('license_check');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });

  it('excludes license_check when disabled', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { ...baseWithLicense.features, license_check: false },
      }),
    });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages.flat()).not.toContain('license_check');
      expect(disabled).toContain('license_check');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
});
