  ...
  it('with all-on base config, produces every feature in correct stages', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: {
          docs: true, eslint: true, example_coverage: true, cloc: true, changelog: true,
    ...
    try {
      ...
      expect(stages[1]?.sort()).toEqual(
        ['changelog', 'cloc', 'docs', 'eslint', 'example_coverage', 'just_test_save', 'typescript'].sort()
      );
...
describe('buildPlan — profiles', () => {
  const allOnConfig = {
    ...
    profiles: {
      fast: {
        features: {
          docs: false, eslint: false, example_coverage: false, cloc: false, changelog: false,
  ...
  it('applies a profile selected via env var', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['docs', 'eslint', 'example_coverage', 'cloc', 'changelog', 'viz_png', 'attw', 'site'].sort()
      );
    ...
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        ...
        profiles: {
          ...
          ci: { features: { docs: true, eslint: true, example_coverage: true, cloc: true, changelog: true,
                            viz_png: true, terser: true, attw: true, site: true } },
        },
      }),
    });
...
describe('buildPlan — file cascade', () => {
  it('overlays build.config.<env>.json on top of base', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { docs: true, eslint: true, example_coverage: true, cloc: true, changelog: true,
                    viz_png: true, terser: true, attw: true, site: true },
      }),
...
describe('buildPlan — env-var and CLI feature overrides', () => {
  ...
  it('--only restricts to listed features, disabling all other optionals', () => {
    ...
    try {
      ...
      expect(disabled.sort()).toEqual(
        ['attw', 'changelog', 'cloc', 'docs', 'example_coverage', 'site', 'terser', 'viz_png'].sort()
      );
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
...
describe('buildPlan — --enable positive test', () => {
  it('--enable lifts a feature that the base config set to false', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { docs: false, eslint: true, example_coverage: true, cloc: true, changelog: true,
                    viz_png: true, terser: true, attw: true, site: true },
      }),
    });
    try {
...
describe('buildPlan — example_coverage feature', () => {
  ...
  it('includes example_coverage in stage 1 when enabled', () => {
    const cwd = makeTmpRepo({ 'build.config.json': JSON.stringify(baseWithExample) });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages[1]).toContain('example_coverage');
      expect(disabled).not.toContain('example_coverage');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });

  it('excludes example_coverage when disabled', () => {
    const cwd = makeTmpRepo({
      'build.config.json': JSON.stringify({
        features: { ...baseWithExample.features, example_coverage: false },
      }),
    });
    try {
      const { stages, disabled } = buildPlan({ cwd, argv: [], env: {} });
      expect(stages.flat()).not.toContain('example_coverage');
      expect(disabled).toContain('example_coverage');
    } finally { rmSync(cwd, { recursive: true, force: true }); }
  });
});
