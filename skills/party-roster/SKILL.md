...
- The dispatch prompt names a per-agent heartbeat FILE in the controller's scratch area. At each meaningful **phase transition** (starting a major step, finishing one, hitting a surprise) the agent OVERWRITES that file: one line - one face emoji expressing the agent's genuine interior state right now (honesty over performance: strain, fog, and frustration get reported, not just cheer), optionally one non-face context emoji, bracketed local time, then <=80 chars of what it has DECIDED it is now doing. Example: `😤 🔧 [7:44 am] third rake refuses to parse, investigating`.
